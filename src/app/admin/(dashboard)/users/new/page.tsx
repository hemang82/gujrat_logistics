...
export default function NewUserPage() {
  ...
  const [autoPassNumber] = useState(() => Math.floor(1000 + Math.random() * 9000));
  const [isPasswordManuallyChanged, setIsPasswordManuallyChanged] = useState(false);

  // Filter branch suggestions based on search
  useEffect(() => {
    if (!branchSearch || branchSearch.trim().length < 1) {
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
    ...
    if (name === 'password') {
      setIsPasswordManuallyChanged(true);
    }

    setFormData(prev => {
      const updated = { ...prev, [name]: value };
      
      if (name === 'email' && !isPasswordManuallyChanged) {
        let prefix = value.split('@')[0].replace(/[^a-zA-Z0-9]/g, '');
        if (prefix) {
          prefix = prefix.charAt(0).toUpperCase() + prefix.slice(1);
          updated.password = `${prefix}@${autoPassNumber}`;
        } else {
          updated.password = '';
        }
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        toast.error(errorData.error || 'Failed to add branch login');
      }
    } catch (error: any) {
      toast.error(error.message || 'An error occurred while creating branch login.');
    } finally {
      setIsLoading(false);
    }
  };
    ...
    <div className="w-full pb-8">
      <div className="mb-4 flex flex-col sm:flex-row sm:items-center justify-between bg-white p-3.5 rounded-xl shadow-sm border border-gray-100 gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-lg md:text-xl font-bold text-gray-800">Add Branch Login</h1>
            <p className="text-xs text-gray-500 mt-0.5">Create a new branch access account</p>
          </div>
        </div>
      </div>
        ...
        <Card className="border border-gray-100 shadow-sm rounded-xl">
          ...
          <CardContent className="p-5 space-y-5">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              ...
              <div className="space-y-1.5">
                <Label className="text-xs font-semibold text-gray-600 uppercase">Password (Auto-Generated) <span className="text-red-500">*</span></Label>
                <Input 
                  name="password" 
                  type="text"
