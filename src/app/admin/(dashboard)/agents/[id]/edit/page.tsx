  ...
  useEffect(() => {
    async function fetchAgent() {
      try {
        ...
        if (data.branch) {
          const fetchedBranches = await fetch('/api/admin/branches').then(r => r.json());
          const b = (fetchedBranches.branches || []).find((br: any) => br._id === data.branch);
          if (b) setBranchSearch(b.name);
        }
      } catch (error: any) {
