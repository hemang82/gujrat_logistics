...
export default function AddChallanPage() {
  ...
  useEffect(() => {
    ...
    // 5. Load All Pending Bookings for LR Autocomplete
  ...
  useEffect(() => {
    async function autoLoadBranchBookings() {
      ...
      if (allPendingBookings.length > 0) {
        ...
        // Map to loaded LRs table structure
        ...
        // Update loaded LRs table (overwriting previous branch bookings)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        toast.error(`Failed to create Challan: ${errorData.error}`);
      }
    } catch (error) {
      toast.error('An error occurred while saving Challan.');
    } finally {
      setIsLoading(false);
    }
  };
    ...
    <div className="w-full pb-8">
      ...
      <div className="mb-4 flex flex-col sm:flex-row sm:items-center justify-between bg-white p-3.5 rounded-xl shadow-sm border border-gray-100 gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-lg md:text-xl font-bold text-gray-800">Add New Challan</h1>
            <p className="text-xs text-gray-500 mt-0.5">Load lorry receipts onto truck transport sheet</p>
          </div>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <CardContent className="p-4 space-y-4">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-7 gap-3">
              ...
              <div className="space-y-1">
                ...
                <Input name="challanNumber" value={formData.challanNumber} onChange={handleChange} placeholder="e.g. 819" className={`h-10 text-sm bg-gray-50 font-bold rounded-lg text-brand-primary ${errors.challanNumber ? 'border-red-500' : 'border-gray-200'}`} />
              ...
              <div className="space-y-1">
                ...
                <div className="space-y-1 md:col-span-2 lg:col-span-1">
                  <Label className="text-xs font-bold text-brand-primary uppercase flex items-center gap-1">
                    Destination Branch
                  </Label>
                ...
                <div className="space-y-1 md:col-span-2">
                  ...
                  <div className="relative">
                    ...
                    {showLrDropdown && lrSuggestions.length > 0 && (
                      <div className="absolute z-50 left-0 right-12 mt-1 bg-white border border-gray-200 rounded-lg shadow-lg max-h-48 overflow-y-auto">
        ...
        <div className="flex gap-2.5">
          ...
          <Button 
            type="button" 
            onClick={handleReset}
            className="h-10 px-6 rounded-lg bg-white border border-red-200 hover:bg-red-50 text-red-600 font-bold text-sm"
          >
            Reset
          </Button>
        </div>
