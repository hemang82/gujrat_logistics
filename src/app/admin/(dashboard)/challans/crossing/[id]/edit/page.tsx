...
export default function EditCrossingPage() {
  ...
  useEffect(() => {
    async function loadResourcesAndChallan() {
      try {
        ...
        const challansRes = await fetch('/api/admin/challans?limit=200');
        const challansData = await challansRes.json();
        if (challansData && challansData.challans) {
          const uniqueAgents = Array.from(
            new Set(challansData.challans.map((c: any) => c.agent).filter(Boolean))
          ) as string[];
        ...
        if (!res.ok) throw new Error('Crossing Challan not found');
        ...
        toast.error('Could not load Crossing Challan details.');
      } finally {
        setIsFetching(false);
      }
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        toast.success('Crossing Challan updated successfully!');
        router.push('/admin/challans/crossing');
        router.refresh();
      } else {
        const errorData = await response.json();
        toast.error(`Failed to update Crossing Challan: ${errorData.error}`);
      }
    } catch (error) {
  ...
  if (isFetching) {
    return (
      <div className="w-full h-96 flex items-center justify-center">
        <p className="text-gray-500 font-medium">Fetching Crossing Challan details...</p>
      </div>
    );
  }
    ...
    <div className="w-full pb-8">
      ...
      <div className="mb-4 flex flex-col sm:flex-row sm:items-center justify-between bg-white p-3.5 rounded-xl shadow-sm border border-gray-100 gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-lg md:text-xl font-bold text-gray-800">Edit Crossing Challan</h1>
            <p className="text-xs text-gray-500 mt-0.5">Modify crossing dispatch sheets and loaded lorry receipts</p>
          </div>
        </div>
      </div>
                    ...
                    {pendingLrs.map((item, index) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className="p-3 text-center text-orange-600">{item.paymentCondition === 'to_pay' ? `₹${item.totalAmount}` : '-'}</td>
                          <td className="p-3 text-center text-emerald-600">{item.paymentCondition === 'paid' ? `₹${item.totalAmount}` : '-'}</td>
                          <td className="p-3 text-center text-blue-600">{item.paymentCondition === 'tbb' ? `₹${item.totalAmount}` : '-'}</td>
                          <td className="p-3 text-center font-bold relative group">
                            <span className="cursor-help border-b border-dotted border-gray-400">₹{item.freight}</span>
