...
export default function EditBranchPage() {
    ...
    async function fetchBranch() {
      try {
        ...
        setFormData({
          ...
          agent: data.agent || '',
          bookingInward: data.bookingInward || 'B/I',
          commiBasis: data.commiBasis || '',
          commiAmount: data.commiAmount?.toString() || '0.00',
          dcBasis: data.dcBasis || '',
          dcAmount: data.dcAmount?.toString() || '0.00',
          lcBasis: data.lcBasis || '',
          lcAmount: data.lcAmount?.toString() || '0.00',
          brnRateBasis: data.brnRateBasis || '',
          brnAmount: data.brnAmount?.toString() || '0.00',
          directData: data.directData || 'No'
        });

        // Set search values
        setBookingInwardSearch(
          data.bookingInward === 'B' ? 'Booking' :
          data.bookingInward === 'I' ? 'Inward' : 'B/I (Both)'
        );
        setDirectDataSearch(data.directData || 'No');
      } catch (error) {
  ...
  useEffect(() => {
    if (formData.pincode && formData.pincode.length === 6 && !isFetching) { // Only fetch if we are not initially loading
  ...
  // Sync agentSearch when agentsList loads or formData.agent changes
  useEffect(() => {
    if (formData.agent && agentsList.length > 0) {
      const match = agentsList.find(a => a.value === ((formData.agent as any)?._id || formData.agent));
      if (match) {
        setAgentSearch(match.label);
      }
    }
  }, [formData.agent, agentsList]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        toast.error(`Failed to update branch: ${errorData.error}`);
      }
    } catch (error) {
    ...
    <div className="w-full pb-8">
      <div className="mb-4 flex flex-col sm:flex-row sm:items-center justify-between bg-white p-3.5 rounded-xl shadow-sm border border-gray-100 gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-lg md:text-xl font-bold text-gray-800">Edit Branch</h1>
            <p className="text-xs text-gray-500 mt-0.5">Modify branch profile and billing configuration</p>
          </div>
        </div>
      </div>
