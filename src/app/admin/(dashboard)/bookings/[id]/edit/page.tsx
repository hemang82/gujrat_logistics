    ...
    async function fetchBooking() {
      try {
        ...
        setFormData({
          ...
          consignorAddress: booking.consignor?.address || '',
          consignorGst: booking.consignor?.gstNumber || '',
          consigneeName: booking.consignee?.name || '',
          consigneePhone: booking.consignee?.phone || '',
          consigneeAddress: booking.consignee?.address || '',
          consigneeGst: booking.consignee?.gstNumber || '',
          pickupLocation: booking.pickupLocation || '',
          deliveryLocation: booking.deliveryLocation || '',
          itemName: booking.material?.itemName || '',
          packagingType: booking.material?.packagingType || '',
          quantity: booking.material?.quantity?.toString() || '',
          weight: booking.material?.weight?.toString() || '',
          chargedWeight: booking.material?.chargedWeight?.toString() || '',
          freightAmount: booking.charges?.freightAmount?.toString() || '',
          hamali: booking.charges?.hamali?.toString() || '',
          surCharge: booking.charges?.surCharge?.toString() || '',
          gstRate: booking.charges?.gstRate?.toString() || '0',
          paymentCondition: booking.paymentCondition || 'to_pay',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        toast.error(`Failed to update LR: ${errorData.error}`);
      }
    } catch (error) {
  ...
  return (
    <div className="w-full pb-10">
      <div className="mb-8 flex justify-between items-center bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
        <div>
          <h1 className="text-2xl md:text-3xl font-bold text-brand-text-primary">Edit LR (Bilty)</h1>
          <p className="text-brand-text-secondary mt-1">Modify the details of the Lorry Receipt below.</p>
        </div>
      ...
      <form onSubmit={handleSubmit} noValidate className="space-y-6">
        ...
        <div className="flex justify-end gap-4 pt-4 border-t border-gray-100">
          ...
          <Button type="submit" disabled={isLoading} className="h-14 px-10 rounded-xl bg-brand-primary hover:bg-brand-primary-dark text-white shadow-lg shadow-brand-primary/20 text-base font-bold transition-all">
            {isLoading ? 'Updating LR...' : 'Save Changes'}
          </Button>
        </div>
      </form>
    </div>
