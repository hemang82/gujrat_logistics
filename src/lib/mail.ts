...
export interface SendBranchUserEmailOptions {
  to: string;
  userName: string;
  email: string;
  password?: string;
...
export async function sendBranchUserEmail({
  to,
  userName,
  email,
  password,
  ...
  if (!host || !user || !pass || !fromEmail) {
    console.warn(
      'SMTP credentials are missing in .env.local. Skipping branch user email delivery.'
    );
    return { success: false, reason: 'SMTP_NOT_CONFIGURED' };
  }
  ...
  try {
    ...
    const isCreate = action === 'create';
    const subject = isCreate
      ? `Welcome to Trust Logistics - Branch Account Created`
      : `Trust Logistics - Branch Account Details Updated`;
        ...
        <div class="container">
          ...
          <div class="content">
            <p class="welcome-text">
              ...
              ${
                isCreate
                  ? `Your branch account has been successfully created on the Trust Logistics platform. Please find your login credentials and branch details below.`
                  : `Your branch account details have been successfully updated on the Trust Logistics platform. Please find the updated information below.`
              }
            </p>
            
            <div class="details-card">
              <h2 class="details-title">Login Credentials</h2>
              <div class="detail-row">
                <span class="detail-label">Login Email:</span>
                <span class="detail-value">${email}</span>
              </div>
            ...
            <div class="details-card">
              <h2 class="details-title">Branch & Account Info</h2>
              <div class="detail-row">
                <span class="detail-label">Assigned Branch:</span>
                <span class="detail-value">${branchName || 'N/A'}</span>
              </div>
              <div class="detail-row">
                <span class="detail-label">Role:</span>
                <span class="detail-value">${getRoleName(role)}</span>
              </div>
              <div class="detail-row">
                <span class="detail-label">Phone Number:</span>
    ...
    console.log(`Branch User Email sent successfully: ${info.messageId}`);
    return { success: true, messageId: info.messageId };
  } catch (error) {
    console.error('Error occurred in sendBranchUserEmail:', error);
    return { success: false, error };
  }
}
